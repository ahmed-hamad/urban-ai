...
const ZONE_TYPES = [
  { id: 'priority',            label: 'منطقة أولوية',                      cls: 'border-red-400 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400' },
  { id: 'non_priority',        label: 'منطقة غير ذات أولوية',              cls: 'border-slate-300 bg-slate-50 dark:bg-slate-500/10 text-slate-600 dark:text-slate-400' },
  { id: 'priority_tourism',    label: 'منطقة أولوية - سياحية',             cls: 'border-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400' },
  { id: 'priority_historical', label: 'منطقة أولوية - تاريخية وأثرية',    cls: 'border-amber-400 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400' },
]

const SYSTEM_SOURCES = ['رصد استباقي', 'رصد الأمين']
...
export default function ReportNew() {
  ...
  useEffect(() => {
    ...
    authFetch(`/api/ingestion/candidates/${candidateId}`)
      ...
      .then(d => {
        ...
        if (!c) return
        setCandidateMedia(c)
        // Priority: URL override (reviewer changed in ConfirmModal) > mapped element > raw AI label
        const resolvedElement = elementFromUrl || c.mapped_urban_element_id || c.suggested_element_type
        setForm(f => ({
          ...f,
          element:          resolvedElement || f.element,
          elementConfirmed: !!resolvedElement,
  ...
  const canNext = () => {
    ...
    if (step === 4) return !!form.district && !!form.description && !!form.monitoringSource
    return true
  }

  const handleAddCustomSource = async () => {
    const label = customSourceInput.trim()
    if (!label) return
    const res = await authFetch('/api/reports/monitoring-sources', {
      method: 'POST',
      body: JSON.stringify({ label }),
    })
    const data = await res?.json()
    if (data?.source) {
      setMonitoringSources(prev => prev.includes(label) ? prev : [...prev, label])
      set('monitoringSource', label)
    }
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null)
    try {
      // Resolve entity UUID: prefer explicit entityId, then lookup by name, then try direct UUID
      const entityUUID = form.entityId
        || entities.find(e => e.name === form.entity)?.id
        || entities.find(e => e.id   === form.entity)?.id
        || null
      ...
      // element_label: Arabic name from regulationData, or the element_id as fallback
      const elementLabel = selectedEl?.name || form.element || null
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {step === 4 && (
        <div className={`${card} rounded-2xl p-6 space-y-4`}>
          ...
          {/* Zone type */}
          <div>
            <label className="text-xs text-slate-500 dark:text-gray-500 mb-2 block">{'تصنيف المنطقة *'}</label>
            <div className="grid grid-cols-2 gap-2">
              {ZONE_TYPES.map(z => (
                <button key={z.id} onClick={() => set('district', z.id)}
                  className={`py-2.5 px-3 rounded-xl border-2 text-xs font-medium text-right transition-all ${form.district === z.id ? z.cls : 'border-slate-200 dark:border-gray-700 text-slate-500 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-gray-800/50'}`}>
                  {z.label}
                </button>
              ))}
            </div>
          ...
          {/* Monitoring source */}
          <div>
            <label className="text-xs text-slate-500 dark:text-gray-500 mb-2 block">{'مصدر الرصد *'}</label>
            <div className="flex flex-wrap gap-2 mb-2">
              {monitoringSources.map(src => (
                <button key={src} onClick={() => { set('monitoringSource', src); setAddingCustomSource(false) }}
                  className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${form.monitoringSource === src ? 'border-blue-500 bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400' : 'border-slate-200 dark:border-gray-700 text-slate-500 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-gray-800'}`}>
                  {src}
                </button>
              ))}
              <button onClick={() => { setAddingCustomSource(true); set('monitoringSource', '') }}
                className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${addingCustomSource ? 'border-purple-500 bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400' : 'border-dashed border-slate-300 dark:border-gray-600 text-slate-400 dark:text-gray-500 hover:border-slate-400'}`}>
                + مصدر آخر
              </button>
            </div>
            {addingCustomSource && (
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="أدخل مسمى مصدر الرصد..."
                  className="flex-1 bg-slate-50 dark:bg-gray-800 border border-purple-300 dark:border-purple-500/50 rounded-lg px-3 py-2 text-sm text-slate-700 dark:text-gray-200 focus:outline-none focus:border-purple-500"
                />
                ...
                <button onClick={() => { setAddingCustomSource(false); setCustomSourceInput('') }}
                  className="px-3 py-2 border border-slate-200 dark:border-gray-700 text-slate-500 text-xs rounded-lg hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors">
                  إلغاء
                </button>
              </div>
            )}
            {form.monitoringSource && (
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                المحدد: <strong>{form.monitoringSource}</strong>
              </p>
            )}
          </div>
        ...
        <div className={`${card} rounded-2xl p-6 space-y-4`}>
          ...
          <div className="space-y-3">
            {[
              ...
              ['الجهة', form.entity || '—'],
              ['تصنيف المنطقة', ZONE_TYPES.find(z => z.id === form.district)?.label || form.district || '—'],
              ['مصدر الرصد', form.monitoringSource || '—'],
