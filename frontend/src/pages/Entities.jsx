...
function EntityModal({ entity, entities, onClose, onSave }) {
  ...
  const [form, setForm] = useState(entity
    ? { name: entity.name || '', type: entity.type || 'municipality', code: entity.code || '',
        parentId: entity.parentId || '', description: entity.description || '',
        phone: entity.phone || '', email: entity.email || '' }
    : { name: '', type: 'municipality', parentId: '', description: '', phone: '', email: '' }
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm"
      ...
      <div className={`${card} rounded-2xl w-full max-w-lg shadow-2xl`}>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          {error && (
            <p className="text-xs text-red-500 bg-red-50 dark:bg-red-500/10 rounded-lg px-3 py-2 text-center">
              {error}
            </p>
          )}
          <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-gray-800">
            ...
            <button type="submit" disabled={saving}
              className="flex-1 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-medium transition-colors flex items-center justify-center gap-2">
...
export default function Entities() {
  ...
  const handleSave = async (form) => {
    const result = editingEntity?.id
      ? await updateEntity(editingEntity.id, form)
      : await addEntity(form)
    if (!result?.error) setEditingEntity(null)
    return result
  }
