...
export default function ReportDetail() {
  ...
  // Duplicate group: reports sharing same location+element from different monitoring sources
  const [reportGroup, setReportGroup] = useState([])
  const { authFetch } = useAuth()
  const isDbId = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)
  useEffect(() => {
    if (!id || !authFetch || !isDbId) return
    authFetch(`/api/reports/${id}/group`)
      .then(r => r?.json())
      .then(d => { if (d?.group) setReportGroup(d.group) })
      .catch(() => {})
  }, [id, authFetch, isDbId])

  // Batch close state
  const [batchConfirm, setBatchConfirm] = useState(null) // { closureType, closureNotes, includeIds }
  ...
  const handleClosureConfirm = ({ closureType, afterPhotos, noticeDuration, letterNumber, letterPhoto }) => {
    ...
    // If this report belongs to a duplicate group, prompt for batch close
    const openGroupMembers = reportGroup.filter(
      r => !['closed_final', 'rejected', 'deleted'].includes(r.status)
    )
    if (isApiReport && openGroupMembers.length > 0) {
      setBatchConfirm({ closureType, closureNotes: afterPhotos.length > 0 ? `${afterPhotos.length} صور مرفقة` : '', openGroupMembers })
      setModal(null)
      return
    }
  ...
  const handleBatchClose = async (includeAll) => {
    if (!batchConfirm) return
    setBatchClosing(true)
    const includeIds = includeAll ? batchConfirm.openGroupMembers.map(r => r.id) : []
    try {
      const res = await authFetch(`/api/reports/${id}/batch-close`, {
        method: 'POST',
        body: JSON.stringify({
          closureType: batchConfirm.closureType,
          closureNotes: batchConfirm.closureNotes,
          includeIds,
        }),
      })
      if (res?.ok) setApiRefreshKey(k => k + 1)
      else alert('فشل الإغلاق الجماعي')
    } catch { alert('حدث خطأ') }
    finally { setBatchClosing(false); setBatchConfirm(null) }
  }
          ...
          <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-500/10 flex items-center justify-center flex-shrink-0">
                <GitBranch size={18} className="text-amber-600 dark:text-amber-400" />
              </div>
              <div>
                <h3 className="font-bold text-slate-800 dark:text-white text-sm">بلاغات مرتبطة مفتوحة</h3>
                <p className="text-xs text-slate-400 dark:text-gray-500">هل المعالجة تخص نفس المخالفة في هذه البلاغات؟</p>
              </div>
            </div>
            <div className="space-y-2">
              {batchConfirm.openGroupMembers.map(r => (
                <div key={r.id} className="flex items-center justify-between bg-slate-50 dark:bg-gray-800 rounded-lg px-3 py-2">
                  <div>
                    <p className="text-xs font-mono font-medium text-slate-700 dark:text-gray-200">{r.report_number || r.id}</p>
                    <p className="text-xs text-slate-400 dark:text-gray-500">{r.monitoring_source || 'مصدر غير محدد'}</p>
                  </div>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30">
                    مفتوح
                  </span>
                </div>
              ))}
            </div>
            <p className="text-xs text-slate-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-500/10 rounded-lg px-3 py-2">
              في حال التأكيد سيتم إرفاق صورة المعالجة وإغلاق جميع البلاغات المرتبطة
            </p>
            <div className="flex gap-2 pt-2">
              <button onClick={() => handleBatchClose(false)} disabled={batchClosing}
                className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-gray-700 text-sm text-slate-600 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors">
                لا — إغلاق هذا فقط
              </button>
              <button onClick={() => handleBatchClose(true)} disabled={batchClosing}
                ...
                {batchClosing && <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                نعم — إغلاق الجميع
              </button>
            </div>
          </div>
