...
export function DataProvider({ children }) {
  ...
  useEffect(() => {
    if (!authUser?.token) { setEntities([]); return }
    authFetch('/api/users/entities')
      .then(res => res?.json())
  ...
  const addReport = (data, currentUser) => {
    ...
    const report = {
      ...
      monitoringSource: data.monitoringSource || '',
  ...
  const addEntity = async (data) => {
    const res = await authFetch('/api/users/entities', {
      method: 'POST',
      body: JSON.stringify({
        name: data.name, type: data.type, code: data.code,
        parentId: data.parentId || null,
    ...
    if (!res) return { error: 'انتهت الجلسة' }
    if (!res.ok) { const err = await res.json(); return { error: err.error || 'فشل إنشاء الجهة' } }
  ...
  const updateEntity = async (id, patch) => {
    const res = await authFetch(`/api/users/entities/${id}`, {
      method: 'PATCH',
      body: JSON.stringify({
        name: patch.name, type: patch.type, code: patch.code,
        parentId: patch.parentId !== undefined ? (patch.parentId || null) : undefined,
    ...
    if (!res) return { error: 'انتهت الجلسة' }
    if (!res.ok) { const err = await res.json(); return { error: err.error || 'فشل تعديل الجهة' } }
    const { entity } = await res.json()
  ...
  const deleteEntity = async (id) => {
    const res = await authFetch(`/api/users/entities/${id}`, { method: 'DELETE' })
    if (!res) return { error: 'انتهت الجلسة' }
    if (!res.ok) { const err = await res.json(); return { error: err.error || 'فشل حذف الجهة' } }
