...
// For managers: returns all users in their entity AND in any child entities (recursive).
router.get('/', requirePermission('view_reports'), async (req, res) => {
  ...
  if (actorRole === 'manager' && eid) {
    params.push(eid)
    // Walk the entity tree downward so managers see users in sub-departments too
    sql = `
      ...
      WHERE u.status != 'inactive'
        AND u.entity_id IN (SELECT id FROM entity_tree)
      ORDER BY u.full_name`
  } else {
    sql = `${USER_SELECT} WHERE u.status != 'inactive' ORDER BY u.full_name`
  }
...
// GET /api/users/entities — accessible to anyone who can manage users or entities
router.get('/entities', async (req, res, next) => {
  const perms = req.user?.permissions || []
  if (req.user?.role !== 'admin' && !perms.includes('manage_users') && !perms.includes('manage_entities')) {
  ...
  const { rows } = await query(
    `SELECT id, name, type, code, is_active,
            parent_id AS "parentId", description, phone, email
     FROM entities WHERE is_active = true ORDER BY name ASC`,
  )
  res.json({ entities: rows })
})

// POST /api/users/entities
router.post('/entities', requirePermission('manage_entities'), async (req, res) => {
  const { name, type, code, parentId, description, phone, email } = req.body
  if (!name || !type) return res.status(400).json({ error: 'الاسم والنوع مطلوبان', code: 'MISSING_FIELDS' })
  if (!['internal', 'external', 'amana', 'municipality', 'agency', 'department'].includes(type))
    return res.status(400).json({ error: 'نوع غير صالح', code: 'INVALID_TYPE' })

  try {
    const { rows: [entity] } = await query(
      `INSERT INTO entities (name, type, code, parent_id, description, phone, email)
       VALUES ($1, $2, $3, $4::uuid, $5, $6, $7)
       RETURNING id, name, type, code, is_active,
                 parent_id AS "parentId", description, phone, email`,
      [name.trim(), type, code?.trim() || null, parentId || null,
       description?.trim() || null, phone?.trim() || null, email?.trim() || null],
    )
    await audit('entity', entity.id, 'created', req.user, { name, type })
    res.status(201).json({ entity })
  } catch (err) {
    if (err.code === '23505') return res.status(409).json({ error: 'الكود مستخدم بالفعل', code: 'CODE_CONFLICT' })
    throw err
  }
})

// PATCH /api/users/entities/:id
router.patch('/entities/:id', requirePermission('manage_entities'), async (req, res) => {
  const { name, type, code, isActive, parentId, description, phone, email } = req.body
  const setClauses = ['updated_at = NOW()']
  const params = []

  if (name !== undefined)        { params.push(name?.trim() || null);        setClauses.push(`name = $${params.length}`) }
  if (type !== undefined)        { params.push(type);                         setClauses.push(`type = $${params.length}`) }
  if (code !== undefined)        { params.push(code?.trim() || null);         setClauses.push(`code = $${params.length}`) }
  if (isActive !== undefined)    { params.push(isActive);                     setClauses.push(`is_active = $${params.length}`) }
  if ('parentId' in req.body)    { params.push(parentId || null);             setClauses.push(`parent_id = $${params.length}::uuid`) }
  if (description !== undefined) { params.push(description?.trim() || null);  setClauses.push(`description = $${params.length}`) }
  if (phone !== undefined)       { params.push(phone?.trim() || null);        setClauses.push(`phone = $${params.length}`) }
  if (email !== undefined)       { params.push(email?.trim() || null);        setClauses.push(`email = $${params.length}`) }

  if (params.length === 0) return res.status(400).json({ error: 'لا توجد حقول للتحديث', code: 'NO_FIELDS' })

  params.push(req.params.id)
  const { rows } = await query(
    `UPDATE entities SET ${setClauses.join(', ')} WHERE id = $${params.length}
     RETURNING id, name, type, code, is_active,
               parent_id AS "parentId", description, phone, email`,
    params,
  )
  if (!rows.length) return res.status(404).json({ error: 'الجهة غير موجودة' })
  await audit('entity', req.params.id, 'updated', req.user, req.body)
  res.json({ entity: rows[0] })
})

// DELETE /api/users/entities/:id — soft delete
router.delete('/entities/:id', requirePermission('manage_entities'), async (req, res) => {
  const { rows } = await query(
    `UPDATE entities SET is_active = false, updated_at = NOW()
     WHERE id = $1 AND is_active = true RETURNING id`,
    [req.params.id],
  )
  if (!rows.length) return res.status(404).json({ error: 'الجهة غير موجودة أو محذوفة بالفعل' })
  await audit('entity', req.params.id, 'deactivated', req.user, {})
